...
LED::LED(ledc_cbs_t *callback) : m_gpioR(g_gpioR), m_gpioG(g_gpioG), m_gpioB(g_gpioB), mp_callbacks(callback)
{
    ...
    //m_ledConf[0].intr_type = LEDC_INTR_FADE_END;
    m_ledConf[0].timer_sel = LEDC_TIMER_1;
    m_ledConf[0].duty = 0;
    ...
    //m_ledConf[1].intr_type = LEDC_INTR_FADE_END;
    m_ledConf[1].timer_sel = LEDC_TIMER_1;
    m_ledConf[1].duty = 0;
    ...
    //m_ledConf[2].intr_type = LEDC_INTR_FADE_END;
    m_ledConf[2].timer_sel = LEDC_TIMER_1;
    m_ledConf[2].duty = 0;
    ...
    for (int i = 0; i < 3; i++)
    {
        ESP_ERROR_CHECK(ledc_channel_config(&m_ledConf[i]));
    }
    ...
    {
        /*
         * NOTE: the ledc_cb_register function was an "undefined reference", which was caused by an error in the driver/ledc.h file
         * where they forgot to add it to the "extern C" block. This was manually pathced in this version of IDF
         */
        ESP_ERROR_CHECK(ledc_cb_register(m_ledConf[i].speed_mode, m_ledConf[i].channel, mp_callbacks, nullptr));
    }
}
